...
import fine_tune.lr_sched as lr_sched
...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for data_iter_step, batch in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        with torch.cuda.amp.autocast():
            ...
            if teacher_model is not None:
                teacher_output = F.softmax(teacher_model(samples), dim=-1)
                # print(f"teacher{teacher_output[0].data.argmax(dim=-1)}label{targets[0]}")
                loss = 0.5 * criterion(outputs, targets) + 0.5 * criterion(outputs, teacher_output)
            else:
    ...
    return {k: meter.global_avg for k, meter in metric_logger.meters.items()}  # gradient_list
...
def evaluate(data_loader, model, device, args=None):
    ...
    for batch in metric_logger.log_every(data_loader, 10, header):
        ...
        if args.class_num >= 5:
            ...
            acc1 = accuracy(output, target, topk=(1, 1))[0]
