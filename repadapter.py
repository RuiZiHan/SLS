...
def forward_vit_block_adapter(self, x):
    ...
    #timm==0.9.10
    x = x + self.drop_path1(self.ls1(self.attn(self.adapter_attn(self.norm1(x)))))
    x = x + self.drop_path2(self.ls2(self.mlp(self.adapter_mlp(self.norm2(x)))))
    return x
...
def set_RepWeight(model, method, dim=8, s=1, args=None):
    if method == 'repblock':
        for _ in model.children():
            if type(_) == timm.models.vision_transformer.Block or type(
                    _) == timm.models.swin_transformer.SwinTransformerBlock:
                ...
                attn_weight, attn_bias = reparameterize(_.adapter_attn.conv_A.weight.squeeze().T, weight_B.T,
                                                        _.adapter_attn.conv_A.bias, _.adapter_attn.conv_B.bias, _.s,
                                                        do_residual=True)
                ...
                mlp_weight, mlp_bias = reparameterize(_.adapter_mlp.conv_A.weight.squeeze().T, weight_B.T,
                                                      _.adapter_mlp.conv_A.bias, _.adapter_mlp.conv_B.bias, _.s,
                                                      do_residual=True)
